export default function ConfigCli({ cliForm, setCliForm, mostrarAlerta, cargarTodo, supabase, lista }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* TABLA DE CLIENTES ESTILO VENTAS */}
<div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-400">
  <div className="p-5 bg-gray-200 border-b-2 border-gray-400 flex justify-between items-center">
    <h3 className="font-black text-slate-800 text-xs uppercase tracking-widest">Base de Datos Clientes</h3>
    <span className="bg-blue-700 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase">Comercial</span>
  </div>
    ...
    <table className="w-full text-left text-[11px] border-collapse">
      <thead>
        <tr className="bg-gray-300 text-slate-800 uppercase font-black">
          ...
          <th className="p-4 border-b-2 border-gray-400">Correo</th>
          <th className="p-4 border-b-2 border-gray-400">Contacto</th>
      ...
      <tbody className="divide-y-2 divide-gray-400">
        {lista?.map((item, index) => (
          <tr key={item.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-200'} hover:bg-yellow-100 transition-colors`}>
            <td className="p-4 font-black text-slate-900 border-l-8 border-blue-700">{item.nombre_completo}</td>
            <td className="p-4 font-bold">{item.nit_cc}</td>
            <td className="p-4 text-blue-700 font-black">{item.correo || '---'}</td>
            <td className="p-4 font-bold">{item.telefono}</td>
            <td className="p-4 uppercase font-black text-slate-600">{item.ciudad}</td>
          </tr>
        ))}
      </tbody>
    </table>
