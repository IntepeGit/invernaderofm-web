...
export default function Dashboard({ listaInvernaderos, datosDespachos, datosEgresos, datosPagos, balancesGrafica }) {
  ...
  // --- CÁLCULOS ESPECÍFICOS DEL INVERNADERO SELECCIONADO ---
  const ventasInv = despachosInv.reduce((acc, d) => acc + (d.total_venta || 0), 0);
  const abonosInv = pagosInv.reduce((acc, p) => acc + (p.monto || 0), 0);
  const carteraInv = ventasInv - abonosInv;
  ...
  const carteraTotalGlobal = totalVentasGlobal - datosPagos.reduce((acc, p) => acc + (p.monto || 0), 0);

  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      
      {/* 1. SECCIÓN DE KPIs GLOBALES */}
      ...
      {/* 3. SELECTOR DE AUDITORÍA (AQUÍ EMPIEZA LA CARTERA POR INVERNADERO) */}
      <div className="bg-slate-900 p-6 rounded-3xl shadow-xl">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic px-1">Auditoría Individual</label>
        <select 
          className="w-full border-2 border-slate-700 p-4 rounded-2xl font-black text-white bg-slate-800 mt-2 outline-none focus:border-blue-500"
          value={invSeleccionado}
          onChange={(e) => setInvSeleccionado(e.target.value)}
        >
          <option value="">-- SELECCIONE UN INVERNADERO PARA DETALLE --</option>
      ...
      {invSeleccionado && (
        <div className="space-y-6 animate-in slide-in-from-top-4 duration-300">
          ...
          {/* KPI DE CARTERA POR INVERNADERO (ESPECÍFICO) */}
          <div className="bg-amber-50 border-2 border-amber-200 p-6 rounded-[30px] flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <span className="text-3xl">📂</span>
              <div>
                <h4 className="font-black text-amber-900 uppercase text-xs tracking-widest">Cartera Pendiente del Invernadero</h4>
                <p className="text-[10px] font-bold text-amber-700 uppercase italic">Saldo sin cobrar de ventas realizadas en este origen</p>
              </div>
            </div>
            <p className="text-3xl font-black text-amber-600 tracking-tighter">{formatoPesos(carteraInv)}</p>
          </div>

          {/* TABLAS DE MOVIMIENTOS DETALLADOS */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* EGRESOS */}
            <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-red-100">
              <div className="bg-red-500 p-3 text-white font-black text-[10px] uppercase tracking-widest">Egresos</div>
            ...
            <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-green-100">
              <div className="bg-green-600 p-3 text-white font-black text-[10px] uppercase tracking-widest">Despachos</div>
              <div className="max-h-64 overflow-y-auto text-[10px]">
                <table className="w-full text-left">
                  <tbody className="divide-y">
                    {despachosInv.map(d => (
                      <tr key={d.id} className="hover:bg-green-50 align-top">
                        <td className="p-2 font-black">#{d.numero_remision}</td>
                        <td className="p-2 italic">
                          {d.detalle_ventas?.map((it, i) => <div key={i}>{it.cantidad} {it.escala}</div>)}
                        </td>
            ...
            <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-blue-100">
              <div className="bg-blue-600 p-3 text-white font-black text-[10px] uppercase tracking-widest">Abonos</div>
              <div className="max-h-64 overflow-y-auto text-[10px]">
                <table className="w-full text-left">
                  <tbody className="divide-y">
                    {pagosInv.map(p => (
      ...
      {/* 4. CARTERA TOTAL GLOBAL (Visible cuando no hay filtro seleccionado) */}
      {!invSeleccionado && (
        <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="font-black text-slate-800 uppercase italic text-sm">Cartera Total Pendiente Global</h4>
              <p className="text-[10px] font-bold text-slate-400 uppercase">Resumen acumulado de toda la granja</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-4xl font-black text-amber-600 tracking-tighter">{formatoPesos(carteraTotalGlobal)}</p>
          </div>
        </div>
      )}
